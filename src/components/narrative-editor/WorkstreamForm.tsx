...
export function WorkstreamForm({
  ...
  async function commit(
    patch: Parameters<typeof updateWorkstreamAction>[1],
  ): Promise<void> {
    try {
      const updated = await updateWorkstreamAction(workstream.id, patch);
      onPatched(updated);
    } catch (err) {
      console.error("[workstream-form] commit failed", err);
    }
  }
  ...
  function handlePhaseChange(value: Key | null): void {
    ...
    void commit({
      phase_id: next,
      // When moving across phases, append at the end of the destination.
      order_index:
        next === null
          ? // Will get a real order_index from the server in the response.
            0
          : 0,
    });
  }
    ...
    >
      ...
      <TextField>
        ...
        <Input
          ...
          onBlur={() => {
            if (draft.name === workstream.name) return;
            if (draft.name.trim().length === 0) return;
            void commit({ name: draft.name });
          }}
          maxLength={NAME_MAX}
          autoFocus
        />
      ...
      <label className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          onBlur={() => {
            if (draft.description === (workstream.description ?? "")) return;
            void commit({ description: draft.description || null });
          }}
          rows={4}
